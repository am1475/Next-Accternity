...
export function FeaturesSectionDemo() {
  return (
    <BackgroundBeamsWithCollision>
      <div className="flex flex-col items-center justify-center pt-10 px-4 sm:px-6 lg:px-8">
        <h1 className="font-bold text-2xl sm:text-4xl md:text-6xl text-center bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-400 py-4">
          Features
        </h1>
        <div className="flex items-center justify-center py-6 sm:py-10 lg:py-20">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8 md:gap-4 lg:gap-6 max-w-7xl mx-auto">
            {grid.map((feature) => (
              <div
                key={feature.title}
                className="relative bg-gradient-to-b dark:from-neutral-900 from-neutral-100 dark:to-neutral-950 to-white p-4 sm:p-6 rounded-3xl overflow-hidden shadow-lg"
              >
                <Grid size={20} />
                <p className="text-base sm:text-lg font-bold text-neutral-800 dark:text-white relative z-20">
                  {feature.title}
                </p>
                <p className="text-sm sm:text-base text-neutral-600 dark:text-neutral-400 mt-2 sm:mt-4 font-normal relative z-20">
                  {feature.description}
                </p>
              </div>
            ))}
